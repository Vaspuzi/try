...
$query = "SELECT users.Fname,f_name, message FROM doctor,users,chat WHERE doctor_id = $did AND doctor_id = CDid AND Cuid = id ";  
...
$query = " INSERT INTO `chat` (`Cid`,`Cuid`, `CDid`, `Message`,`sender`) VALUES (NULL,'$pid', '$sender', '$message','1')";
if($run = mysqli_query($conn,$query)){
return true;
}else{
	return false;
